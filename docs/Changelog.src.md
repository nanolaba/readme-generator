...
- **`asset` widget**: resolves to a per-language asset path (image, file, URL) configured via `<\!--\@asset.NAME.LANG=...-->` markers, with fallback to a shared `<\!--\@asset.NAME=...-->` value when no language-specific override exists. Replaces the noisy inline `\${en:'./en.png', ru:'./ru.png'}` form when the same asset is referenced from many places.<!--en-->
- **Виджет `asset`**: возвращает путь к ресурсу (изображению, файлу, URL) для конкретного языка; значения задаются маркерами `<\!--\@asset.NAME.LANG=...-->`, а при отсутствии локализованного значения подставляется общий `<\!--\@asset.NAME=...-->`. Заменяет многословный инлайн-вариант `\${en:'./en.png', ru:'./ru.png'}`, когда один ресурс упоминается в нескольких местах.<!--ru-->
