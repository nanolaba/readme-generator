...
Текущая версия разработки — **1.1-SNAPSHOT**.
...
1. [Способы запуска программы](#способы-запуска-программы)
	...
	3. [Использование в качестве GitHub Action](#использование-в-качестве-github-action)
		...
		3. [Выходные значения](#выходные-значения)
		4. [Примеры](#примеры)
			1. [Базовая генерация](#базовая-генерация)
			2. [Проверка расхождений на PR](#проверка-расхождений-на-pr)
		5. [Проекты с несколькими шаблонами](#проекты-с-несколькими-шаблонами)
		6. [Отключение встроенного setup-java](#отключение-встроенного-setup-java)
		7. [Закрепление версии action](#закрепление-версии-action)
		8. [Решение проблем](#решение-проблем)
	4. [Использование в качестве java-библиотеки](#использование-в-качестве-java-библиотеки)
2. [Синтаксис шаблона](#синтаксис-шаблона)
	...
	7. [Виджеты](#виджеты)
		...
		8. [Виджет 'fileTree'](#виджет-filetree)
...
**Вариант A — CLI.** [Скачайте](https://github.com/nanolaba/readme-generator/releases/tag/v1.0) автономный jar, распакуйте архив и запустите:
...
[Скачайте](https://github.com/nanolaba/readme-generator/releases/tag/v1.0) последнюю 
...
NRG поставляется в виде composite GitHub Action, поэтому любой репозиторий
может перегенерировать мультиязычные README в CI без локальной установки
Maven или Java. Action настраивает Java (опционально), скачивает
запрошенный релизный zip, извлекает `nrg.jar` и запускает его на указанных
шаблонах — превращая поддержку README в одношаговый workflow.
...
|---|---|---|
| `file` | Путь к шаблону .src.md (относительно working-directory). Для нескольких файлов используйте `files`. | — |
| `files` | Многострочный список шаблонов (по одному в строке). Несовместим с `file`. | — |
...
Перегенерация README при каждом push в `main`:
...
##### Проверка расхождений на PR

Завершить сборку с ошибкой, если автор изменил `README.md` напрямую, не перегенерировав его из `README.src.md`:
...
Рецепты «только валидация» и «авто-commit» (открытие PR через `peter-evans/create-pull-request`) доступны в репозитории action — см. [examples](https://github.com/nanolaba/nrg-action/tree/main/examples). Примечание: ссылка действительна с момента публикации отдельного репозитория.

#### Проекты с несколькими шаблонами

Передайте heredoc-список в параметр `files:` (по одному пути в строке). Все файлы обрабатываются за один вызов action; jar скачивается один раз. `file` и `files` несовместимы — задавайте только один из них.
...
Если в workflow уже установлена Java (например, для сборки Maven), можно отключить встроенный шаг `actions/setup-java@v4`. Параметры composite action — строки, поэтому используйте `"false"` в кавычках, а не YAML-булево:
...
Используйте `@v1` для автообновления в пределах major-версии v1 (рекомендуется), `@v1.0` для жёсткой фиксации minor-версии или `@<full-sha>` для пина конкретного коммита (наиболее безопасно; требуется некоторыми политиками supply chain).

#### Решение проблем

Большинство ошибок в CI делятся на три группы: проблемы скачивания (убедитесь, что `nrg-version` есть на странице [Releases](https://github.com/nanolaba/readme-generator/releases)), регрессии раскладки zip (откройте issue, если появилось `nrg.jar not found inside …`), и платформенные особенности. Самая частая особенность — расхождение перевода строк на `windows-latest`: `mode: check` показывает diff, которого нет локально.

- **Расхождение перевода строк на Windows-runners** — добавьте `.gitattributes` со строкой `* text=auto eol=lf` и перекоммитьте сгенерированные файлы.
- **Windows: `unzip: command not found`** — git-bash на `windows-latest` обычно содержит `unzip`; в редких образах установите его через `choco install unzip` в предыдущем шаге.

Полный листинг для Marketplace — в отдельном репозитории. Источник: [`nanolaba/nrg-action`](https://github.com/nanolaba/nrg-action) (планируемый репозиторий).
...
Скачайте JAR из [Maven Central](https://repo1.maven.org/maven2/com/nanolaba/readme-generator/1.0)
...
```markdown
Last updated: 26.04.2026 00:58:42
```
...
- Пока без видимых пользователю изменений.
