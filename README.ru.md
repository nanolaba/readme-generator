...
1. [Способы запуска программы](#способы-запуска-программы)
	...
	3. [Использование в качестве GitHub Action](#использование-в-качестве-github-action)
		...
		4. [Примеры](#примеры)
			...
			2. [Проверка расхождений в PR](#проверка-расхождений-в-pr)
...
2. [Синтаксис шаблона](#синтаксис-шаблона)
	...
	5. [Значения из package.json](#значения-из-packagejson)
	6. [Значения из Gradle](#значения-из-gradle)
	7. [Multilanguage support](#multilanguage-support)
	8. [Игнорирование фрагментов](#игнорирование-фрагментов)
	9. [Виджеты](#виджеты)
...
Этот компонент возвращает путь к ресурсу (изображению, файлу, URL и т. п.) для конкретного языка,
а при отсутствии локализованного значения подставляет общее значение по умолчанию.
Позволяет избежать многословной конструкции `...`, когда один ресурс упоминается в нескольких местах.

Значения задаются обычными свойствами NRG:
`asset.<name>.<lang>` определяет значение для конкретного языка;
`asset.<name>` (без суффикса языка) используется как значение по умолчанию, если для языка нет переопределения.
...
<tr><th>Результат для любого другого языка</th></tr>
<tr><td>
...
| Наименование | Описание                                                                                                       | Значение по умолчанию |
|:-------------------------------:|------------------------------------------------------------------------------------------------------------------------------------------|:-------------------------------------------------:|
|              name               | Идентификатор ресурса (соответствует суффиксу в `asset.<name>` и `asset.<name>.<lang>`) |       —       |

Если не определены ни `asset.<name>.<lang>`, ни `asset.<name>`, виджет регистрирует ошибку и выдает пустую строку.
