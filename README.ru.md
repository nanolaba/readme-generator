...
Также доступен под коротким именем `${widget:toc(...)}` — обе формы рендерятся одинаково.
...
Этот виджет решает, попадает ли содержимое в результат — на основе условия. Существует
в двух формах: **блочная** охватывает открывающий тег `${widget:if(cond='…')}` и парный
`${widget:endIf}` (строки между ними попадают в вывод только при истинном условии — при
ложном целый блок, включая вложенные виджеты, отбрасывается до запуска per-line-конвейера,
поэтому виджеты в мёртвой ветке не выполняются никогда); **однотеговая** форма
`${widget:if(cond='…', text='…')}` выдаёт `text` одной строкой при истинном условии
или пустую строку при ложном.
...
Однотеговая форма — короткая условная вставка в одну строку:

```markdown
${widget:if(cond='endsWith(${devVersion}, -SNAPSHOT)', text='Snapshot build — expect breaking changes.')}
```

`\n` и `\\` внутри `text=` интерпретируются (`\n` → реальный перевод строки, `\\` → одиночный обратный слэш). Подстановка свойств и языковых конструкций внешнего шаблона выполняется до запуска виджета, поэтому `${var}` и языковые конструкции внутри `text=` работают как обычно.
...
- **Виджет `if` — новая однотеговая форма**: `${widget:if(cond='…', text='…')}` выдаёт `text` одной строкой при истинном условии, либо пустую строку — удобно для коротких условных вставок, когда полноценная блочная форма (`${widget:if(cond='…')}` … `${widget:endIf}`) избыточна. `\n` и `\\` внутри `text=` интерпретируются, подстановка переменных и языковых конструкций внешнего шаблона выполняется до запуска виджета. Блочная форма не изменилась. Внутренне обе формы теперь живут на едином базовом классе `BlockWidget`, общем с `details`; standalone-классы `IfBlockProcessor` / `DetailsBlockProcessor` удалены (без user-visible эффекта — внутренний рефакторинг).
- **Короткий алиас `toc` для `tableOfContents`**: `${widget:toc(...)}` теперь работает как синоним `${widget:tableOfContents(...)}` — генерация якорей, параметры и итоговый вывод побайтно совпадают. Основным именем остаётся `tableOfContents`. Алиасы — общая возможность системы виджетов: сторонние виджеты могут декларировать свои через новый default-метод `NRGWidget.getAliases()`.
