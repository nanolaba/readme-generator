...
2. [Синтаксис шаблона](#синтаксис-шаблона)
	...
	4. [Игнорирование фрагментов](#игнорирование-фрагментов)
	5. [Виджеты](#виджеты)
...
Чтобы пометить фрагмент как авторскую заметку, которая не должна попасть ни в один сгенерированный файл, используйте маркеры `nrg.ignore`. Они работают как в основном шаблоне, так и внутри импортированных файлов.
...
Если у `<!--nrg.ignore.begin-->` нет парного `<!--nrg.ignore.end-->`, выводится ошибка в лог, а все строки от открывающего маркера до конца файла отбрасываются. Одиночный
`<!--nrg.ignore.end-->` без открывающего тоже логируется как ошибка и удаляется из вывода.
